export const createService = (
  prefix,
  {
    ...
    albSecurityGroup,
    commonEnv,
    port = 4848, // Default to the view-syncer port, but allow override
  }: {
  ...
  return $util.all(
    ...
    const resolvedEnv = Object.fromEntries(
      Object.entries(commonEnv).map(([name, _value], i) => [name, resolvedValues[i]])
    );
    ...
    const containerDefinition = {
      ...
      healthCheck: {
        command: [`CMD-SHELL`, `curl -f http://localhost:${port}/keepalive || exit 1`],
        interval: 5,
        timeout: 3,
        retries: 3,
        startPeriod: 300,
    ...
    return new aws.ecs.Service(`${prefix}Service`, {
      cluster: cluster.arn,
